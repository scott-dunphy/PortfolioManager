...
with st.expander("Loan Details"):
    ...
    if loan_exists:
        ...
        interest_only_period = st.number_input('Interest Only Period (months)', min_value=0, value=float(selected_property.loan.interest_only_period) if selected_property.loan else 0)
        amortization_period = st.number_input('Amortization Period (months)', min_value=0, value=float(selected_property.loan.amortization_period) if selected_property.loan else 0)
